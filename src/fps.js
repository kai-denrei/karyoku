...
export const stepCost = { ema: 0, worst: 0 };
export function noteStep(ms) { stepCost.ema += (ms - stepCost.ema) * 0.05; if (ms > stepCost.worst) stepCost.worst = ms; }
export function tickFps(now, renderer = null, scene = null) {
  ...
  if (now - last >= 1000) {
    ...
    if (perf && renderer && now - perfAt >= 2000) {
      ...
      console.log(`[perf] step=${stepCost.ema.toFixed(2)}ms worst=${stepCost.worst.toFixed(1)}ms`); stepCost.worst = 0;
